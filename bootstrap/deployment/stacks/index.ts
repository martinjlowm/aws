  ...
  constructor(app: App, region: string) {
    ...
    new StackSet(this, 'cdk-bootstrap-service-managed', {
      ...
      target: StackSetTarget.fromOrganizationalUnits({
        ...
        // Every unit that holds an account CDK deploys into. Management is on
        // the list for the domains account: a service-managed stack set cannot
        // deploy to the organization's management account, so naming the unit
        // reaches the accounts beside it and skips the one the self-managed
        // stack set below covers instead.
        organizationalUnits: [
          org.organizationalUnits.management,
          org.organizationalUnits.applications,
          org.organizationalUnits.operations,
        ],
      }),
